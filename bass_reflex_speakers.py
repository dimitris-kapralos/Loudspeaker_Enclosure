...
class BassReflexEnclosure:
    ...
    def calculate_diaphragms_radiation_impedance_Za1(self, f):
        ...
        z11 = z22 = R_0 * SOUND_CELERITY * ((1 - (J1**2/ (k * self.lsp.a))) + 1j* (H1/ (k * self.lsp.a))) 
        ...
        for m in range(self.truncation_limit+1):
            for n in range(self.truncation_limit+1):
                ...
                term6 = spherical_jn(m + n, k * self.d1) + 1j * spherical_yn(m + n, k * self.d1)
    ...
    def calculate_box_impedance_for_rectangular_piston_Z11 (self, f):
        ...
        for n in range(1, self.truncation_limit + 1):
            k0n = np.sqrt(k**2 - (0*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
            term1 = (k)/(k0n*n**2) * np.cos(n*np.pi*self.y1/self.ly) * np.cos(n*np.pi*self.y1/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
            term2 = ((k0n*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(k0n*self.lz)) / (1 + 1j * (( k0n*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(k0n*self.lz))
            sum1 += term1 * term2

        for m in range(1, self.truncation_limit + 1):
            km0 = np.sqrt(k**2 - (2* m*np.pi/self.lx)**2 - (0*np.pi/self.ly)**2)
            term1 = (k)/(km0*m**2) * np.sin(m*np.pi*self.a1/self.lx) * np.sin(m*np.pi*self.a1/self.lx) 
            term2 = ((km0*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(km0*self.lz)) / (1 + 1j * (( km0*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(km0*self.lz))
            sum2 += term1 * term2  

        for m in range(1, self.truncation_limit + 1):
            for n in range(1, self.truncation_limit + 1):
                kmn = np.sqrt(k**2 - (2* m*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
                term1 = (k)/(kmn*m**2*n**2) * np.sin(m*np.pi*self.a1/self.lx) * np.sin(n*np.pi*self.a1/self.lx) * np.cos((m*np.pi*self.y1)/ (self.ly)) 
                term2 = np.cos(n*np.pi*self.y1/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
                term3 = ((kmn*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(kmn*self.lz)) / (1 + 1j * (( kmn*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(kmn*self.lz))
        
            sum3 += term1 * term2 * term3

        Z11 = R_0*SOUND_CELERITY *( 1 /(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) +  (8*self.ly)/(np.pi**2 * self.b1 * self.b1* self.lx) * sum1  + 
                                    (2*self.lx)/(np.pi**2 * self.a1 *self.a1* self.ly) *sum2 + (16*self.lx*self.ly)/(np.pi**4 * self.a1 * self.a1 * self.b1 * self.b1) * sum3 ) 
        return  Z11
    ...
    def calculate_box_impedance_for_rectangular_piston_Z12(self, f):
        ...
        for n in range(1, self.truncation_limit + 1):
            k0n = np.sqrt(k**2 - (0*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
            term1 = (k)/(k0n*n**2) * np.cos(n*np.pi*self.y1/self.ly) * np.cos(n*np.pi*self.y2/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
            term2 = ((k0n*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(k0n*self.lz)) / (1 + 1j * (( k0n*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(k0n*self.lz))
            sum1 += term1 * term2
        ...
        for m in range(1, self.truncation_limit + 1):
            for n in range(1, self.truncation_limit + 1):
                kmn = np.sqrt(k**2 - (2* m*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
                term1 = (k)/(kmn*m**2*n**2) * np.sin(m*np.pi*self.a1/self.lx) * np.sin(n*np.pi*self.a1/self.lx) * np.cos((m*np.pi*self.y1)/ (self.ly)) 
                term2 = np.cos(n*np.pi*self.y2/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
                term3 = ((kmn*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(kmn*self.lz)) / (1 + 1j * (( kmn*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(kmn*self.lz))
        
            sum3 += term1 * term2 * term3

        Z12 = R_0*SOUND_CELERITY *( 1 /(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) +  (8*self.ly)/(np.pi**2 * self.b1 * self.b1* self.lx) * sum1  + 
                                    (2*self.lx)/(np.pi**2 * self.a1 *self.a1* self.ly) *sum2 + (16*self.lx*self.ly)/(np.pi**4 * self.a1 * self.a1 * self.b1 * self.b1) * sum3 ) 
        return  Z12
    ...
    def calculate_box_impedance_for_rectangular_piston_Z22(self, f):
        ...
        for n in range(1, self.truncation_limit + 1):
            k0n = np.sqrt(k**2 - (0*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
            term1 = (k)/(k0n*n**2) * np.cos(n*np.pi*self.y2/self.ly) * np.cos(n*np.pi*self.y2/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
            term2 = ((k0n*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(k0n*self.lz)) / (1 + 1j * (( k0n*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(k0n*self.lz))
            sum1 += term1 * term2
        ...
        for m in range(1, self.truncation_limit + 1):
            for n in range(1, self.truncation_limit + 1):
                kmn = np.sqrt(k**2 - (2* m*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
                term1 = (k)/(kmn*m**2*n**2) * np.sin(m*np.pi*self.a1/self.lx) * np.sin(n*np.pi*self.a1/self.lx) * np.cos((m*np.pi*self.y2)/ (self.ly)) 
                term2 = np.cos(n*np.pi*self.y2/self.ly) * np.sin((n*np.pi*self.b1)/ (2*self.ly)) * np.sin((n*np.pi*self.b1)/(2*self.ly))
                term3 = ((kmn*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(kmn*self.lz)) / (1 + 1j * (( kmn*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(kmn*self.lz))
        
            sum3 += term1 * term2 * term3

        Z22 = R_0*SOUND_CELERITY *( 1 /(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) +  (8*self.ly)/(np.pi**2 * self.b1 * self.b1* self.lx) * sum1  + 
                                    (2*self.lx)/(np.pi**2 * self.a1 *self.a1* self.ly) *sum2 + (16*self.lx*self.ly)/(np.pi**4 * self.a1 * self.a1 * self.b1 * self.b1) * sum3 ) 
        return  Z22
    ...
    def calculate_box_impedance_for_circular_piston_Z11(self, f):
        ...
        for m in range(self.truncation_limit+1):
            for n in range(self.truncation_limit+1):
                kmn = np.sqrt(k**2 - (m*np.pi/self.lx)**2 - (n*np.pi/self.ly)**2)
                delta_m0 = 1 if m == 0 else 0
                delta_n0 = 1 if n == 0 else 0
                term1 = ((kmn*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(kmn*self.lz)) / (1 + 1j * (( kmn*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(kmn*self.lz))
                term2 = (2 - delta_m0) * (2 - delta_n0) / (kmn * (n**2 * self.lx**2 + m**2 * self.ly**2) + delta_m0 * delta_n0)
                term3 = np.cos((m*np.pi*self.x1)/self.lx) * np.cos((n*np.pi*self.y1)/self.ly) * j1((np.pi * self.lsp.a * np.sqrt(n**2 * self.lx**2 + m**2 * self.ly**2))/(self.lx*self.ly))
                term4 = np.cos((m*np.pi*self.x1)/self.lx) * np.cos((n*np.pi*self.y1)/self.ly) * j1((np.pi * self.lsp.a * np.sqrt(n**2 * self.lx**2 + m**2 * self.ly**2))/(self.lx*self.ly))
                sum_mn += term1 * term2 * term3 * term4 

        Z11 = (R_0 * SOUND_CELERITY * ( (self.lsp.Sd * self.lsp.Sd)/(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) + 4 * k*self.lsp.a*self.lsp.a*self.lx*self.ly* sum_mn)) / self.lsp.Sd**2   
    ...
    def calculate_box_impedance_for_circular_piston_Z22(self, f):
        ...
        for m in range(self.truncation_limit+1):
            for n in range(self.truncation_limit+1):
                ...
                term1 = ((kmn*Zs)/(k*R_0*SOUND_CELERITY) + 1j * np.tan(kmn*self.lz)) / (1 + 1j * (( kmn*Zs)/(k*R_0*SOUND_CELERITY)) * np.tan(kmn*self.lz))
                term2 = (2 - delta_m0) * (2 - delta_n0) / (kmn * (n**2 * self.lx**2 + m**2 * self.ly**2) + delta_m0 * delta_n0)
                term3 = np.cos((m*np.pi*self.x2)/self.lx) * np.cos((n*np.pi*self.y2)/self.ly) * j1((np.pi * self.lsp.a * np.sqrt(n**2 * self.lx**2 + m**2 * self.ly**2))/(self.lx*self.ly))
                term4 = np.cos((m*np.pi*self.x2)/self.lx) * np.cos((n*np.pi*self.y2)/self.ly) * j1((np.pi * self.lsp.a * np.sqrt(n**2 * self.lx**2 + m**2 * self.ly**2))/(self.lx*self.ly))
                sum_mn += term1 * term2 * term3 * term4 

        Z22 = (R_0 * SOUND_CELERITY * ( (self.lsp.Sd * self.lsp.Sd)/(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) + 4 * k*self.lsp.a*self.lsp.a*self.lx*self.ly* sum_mn)) / self.lsp.Sd**2   
    ...
    def calculate_box_impedance_for_circular_piston_Z12(self, f):
        ...
        Z12 = (R_0 * SOUND_CELERITY * ( (self.lsp.Sd * self.lsp.Sd)/(self.lx*self.ly) * (((Zs/(R_0*SOUND_CELERITY)) + 1j * np.tan(k*self.lz)) / (1 + 1j * ((Zs/(R_0*SOUND_CELERITY)) * np.tan(k*self.lz)))) + 4 * k*self.lsp.a*self.lsp.a*self.lx*self.ly* sum_mn)) / self.lsp.Sd**2   
    ...
    def calculate_port_impedance_Za2(self, f, r_d):
        ...
        Xs_a2 = (((2 * r_d * SOUND_CELERITY) / (np.sqrt(np.pi))) *
                    ...
                    (1 - np.sinc(self.q * k*self.lx))/(k*self.lx) + sum_Xs))
    ...
    def calculate_impedance_response(self, frequencies):
        ...
        for i in range(len(frequencies)):
            ...
            if (self.speaker_type == 'rectangular' and self.port_shape == 'rectangular'):
                ...
                Z11 = self.calculate_box_impedance_for_rectangular_piston_Z11(frequencies[i])
                Z12 = self.calculate_box_impedance_for_rectangular_piston_Z12(frequencies[i])
                Z22 = self.calculate_box_impedance_for_rectangular_piston_Z22(frequencies[i])
                Z21 = Z12
                ...
                Z11 = self.calculate_box_impedance_for_circular_piston_Z11(frequencies[i])
                Z12 = self.calculate_box_impedance_for_circular_piston_Z12(frequencies[i])
                Z22 = self.calculate_box_impedance_for_circular_piston_Z22(frequencies[i])
                Z21 = Z12
...
ax1.semilogx(frequencies, response_1,
             label='1 Rect. Loudspeaker with Rect. port')
ax1.semilogx(frequencies, response_2,
             label='1 Circ. Loudspeaker with Rect. port')
ax1.semilogx(frequencies, response_3,
...
ax2.semilogx(frequencies, Ze_3, 
             label='2 Circ. Loudspeakers with Circ. port')

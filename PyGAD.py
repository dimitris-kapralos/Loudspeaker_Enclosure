...
num_ports = [0.00000000000001, 4, 8, 12, 16, 20]
...
def fitness_func(ag_instance, solution, solution_idx):
    ...
    logger.log(logging.INFO, f"{edge=}")
    logger.log(logging.INFO, f"{diode_param=}")
    logger.log(logging.INFO, f"{ports=}")

    sound_power = calculate_sound_power(loudspeaker, clb_par, edge, diode_param, ports)
    ...
    for i in range(len(edge)):
        ...
        if open_angle < 21 or open_angle > 26.6:
            sound_power[i] = 0
...
ga_instance = pygad.GA(
    ...
    # random_seed=42,  # Set the seed for PyGAD
    logger=logger,
)

...
from scipy.special import jv, gamma, spherical_jn, spherical_yn, j1
...
t = 0.44  # length of the port in m
Va = 17.2  # volume of the enclosure without the volume of lining material in m^3
Vm = 6.1  # volume of the lining material in m^3
...
Sp = 0.00502  # area of the port in m^2
...
y2 = 0.52  # distance from the center of the diaphragm to the box wall
lz = 0.192  # length of the box
lx = 0.15  # width of the box
ly = 0.63  # height of the box
...
def calculate_Za1(f, a1, r_0, c, d1, truncation_limit):
    ...
    Za1 = (a1**2 * z11 + 2 * a1 * a1 * z12) / (a1**2 + a1**2)
    return Za1


# calculate the radiation impedance of the box based on equations 7.131
...
def calculate_box_impedance_Zab(f):
    ...
    Rab = 4480 / ((1 + Va / (1.4 * Vm)) ** 2 + (2 * np.pi * f) ** 2 * 4480**2 * CAA**2)
    # print(Rab)
...
for i in range(len(frequencies)):
    ...
    # Calculate the radiation impedance of the diaphragms based on Mellow's equation
    ...
    Z11 = calculate_box_impedance_for_circular_piston_Zxy(
        frequencies[i], a1, a1, x1, x1, y1, y1, Sd, Sd
    )
    Z12 = calculate_box_impedance_for_circular_piston_Zxy(
        frequencies[i], a1, a2, x1, x1, y1, y2, Sd, Sd
    )
    Z22 = calculate_box_impedance_for_circular_piston_Zxy(
        frequencies[i], a2, a2, x1, x1, y2, y2, Sd, Sd
    )
    Z21 = calculate_box_impedance_for_circular_piston_Zxy(
        frequencies[i], a2, a1, x1, x1, y2, y1, Sd, Sd
    )

# Description: This script calculates the impedance and response of 2 circular loudspeakers with a rectangular bass reflex port
...
def Q(x):
  ...
  first_term = jv(1, x) 
  second_term = jv(0, x)
  return (2 * first_term) / ( x * second_term)
...
for i in range(len(frequencies)):
    ...
    r_d = (8*m)/(1j * 2 * np.pi * frequencies[i] * (1 + 4 * Bu) * a_2**2)

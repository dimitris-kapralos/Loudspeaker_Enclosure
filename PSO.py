...
def calculate_sound_power(
    loudspeaker, clb_parameters, pentagon_edge, diode_parameters, num_ports
):

    enclosure = DodecahedronBassReflexEnclosure(
        clb_parameters, pentagon_edge, diode_parameters, loudspeaker
    )
...
optimizer = ps.single.GlobalBestPSO(
    n_particles=35, dimensions=4, options=options, bounds=bounds
)
...
best_speaker_idx, best_edge_idx, best_diode_idx, best_ports_idx = best_solution_indices
...
plt.bar(
    np.arange(len(power_1_3_octave)), power_1_3_octave, width=bar_width, align="center"
)
